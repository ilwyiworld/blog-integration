layui.use(['element', 'jquery', 'form', 'layedit'], function () {
	...
	var editorOptions = {
        height: 150,
        tool: ['face', '|', 'left', 'center', 'right', '|', 'strong', 'italic' , 'underline', 'del', '|' ,'link', 'unlink' ],
    }
    //评论和留言的编辑器
    var editIndex = layedit.build('remarkEditor',editorOptions);
    //评论和留言的编辑器的验证
    form.verify({
    ...
    form.on('submit(formLeaveMessage)', function (data) {
        ...
        setTimeout(function () {
            ...
            $('#remarkEditor').val('');
            editIndex = layui.layedit.build('remarkEditor', editorOptions);
